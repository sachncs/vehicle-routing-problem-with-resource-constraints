...
const alnsIterInput = document.getElementById('alnsIter');
...
function draw() {
    ...
    currentProblem.customers.forEach(c => {
    ...
    if (currentSolution) {
        ...
        currentSolution.routes.forEach((route, i) => {
            ...
            // Draw direction arrows (simplified)
        });
    }

    // Draw nodes
    Object.values(currentProblem.nodes).forEach(node => {
...
solveBtn.onclick = async () => {
    ...
    document.getElementById('makespan').innerText = solution.makespan.toFixed(2);
    document.getElementById('execTime').innerText = `${Math.round(endTime - startTime)}ms`;
    solveBtn.disabled = false;
    
    draw();
};

customerCountInput.oninput = (e) => {
    document.getElementById('customerCountVal').innerText = e.target.value;
    generateProblem();
};
vehicleCountInput.oninput = (e) => {
    document.getElementById('vehicleCountVal').innerText = e.target.value;
    generateProblem();
};
